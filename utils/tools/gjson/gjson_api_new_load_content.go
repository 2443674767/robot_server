...
func loadContentWithOptions(data []byte, options Options) (*Json, error) {
	...
	options.Type = ContentType(gstr.TrimLeft(string(options.Type), "."))
	switch options.Type {
	...
	default:
		err = fmt.Errorf(`unsupported type "%s" for loading`, options.Type)
	}
	if err != nil {
		return nil, err
	}
	...
	switch result.(type) {
	case string, []byte:
		return nil, fmt.Errorf(`json decoding failed for content: %s`, data)
	}
...
func checkDataType(data []byte) (ContentType, error) {
	...
	default:
		return "", errors.New("unable auto check the data format type")
	}
}
