...
func (c *Converter) Uint64(any any) (uint64, error) {
	...
	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		val := rv.Int()
		if val < 0 {
			return uint64(val), fmt.Errorf(`cannot convert negative value "%d" to uint64`, val)
		}
	...
	case reflect.Float32, reflect.Float64:
		val := rv.Float()
		if val < 0 {
			return uint64(val), fmt.Errorf(`cannot convert negative value "%f" to uint64`, val)
		}
		return uint64(val), nil
	case reflect.Bool:
		if rv.Bool() {
	...
	case reflect.Slice:
		...
		return 0, fmt.Errorf(`unsupport slice type "%s" for converting to uint64`, rv.Type().String())
	case reflect.String:
		var s = rv.String()
		// Hexadecimal
	...
	return 0, fmt.Errorf(`unsupport value type "%s" for converting to uint64`, reflect.TypeOf(any).String())
}
