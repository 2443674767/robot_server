...
func DBDump(opts ...DumpOption) error {
	dbConf, err := gcfg.Instance().Get(ctx, "database.default")
	if err != nil {
		return err
	}
	dbConf_arr := gconv.Map(dbConf)
	dbprefix := gconv.String(dbConf_arr["prefix"])
	dsn := drivers.DbDSN(dbConf_arr) //从app\dal\model获取dsn链接
	// 打印开始
	start := time.Now()
...
// 移除表前缀
func removePrefix(table, prefix string) string {
...
func writeTableStruct(db *sql.DB, table string, buf *bufio.Writer) error {
	dbprefix, _ := gcfg.Instance().Get(ctx, "database.default.prefix")
	// 导出表结构
	buf.WriteString("-- ----------------------------\n")
	buf.WriteString(fmt.Sprintf("-- Table structure for %s\n", removePrefix(table, dbprefix.String())))
	buf.WriteString("-- ----------------------------\n")
	...
	if createTableSQL != "" {
		createTableSQL = strings.ReplaceAll(createTableSQL, "NOT EXISTS `"+dbprefix.String(), "NOT EXISTS `")
	}
...
func writeTableData(db *sql.DB, table string, buf *bufio.Writer) error {
	...
	for _, row := range values {
		ssql := "INSERT INTO `" + removePrefix(table, dbprefix.String()) + "` VALUES ("
...
// 导入sql数据文件
func ImportSql(SqlPath string) (sql.Result, error) {
	dbConf, err := gcfg.Instance().Get(ctx, "database.default")
	if err != nil {
		return nil, err
	}
	dbConf_arr := gconv.Map(dbConf)
	dbtype := gconv.String(dbConf_arr["type"])
	dsn := drivers.DbDSN(dbConf_arr) //从app\dal\model获取dsn链接
	driverName := dbtype
	if dbtype == "pgsql" {
		driverName = "postgres"
	}
	// 必须添加 multiStatements=true 支持多条SQL
	if dbtype == "mysql" {
