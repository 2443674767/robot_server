...
func Copy(src string, dst string, option ...CopyOption) error {
	...
	if srcStatErr != nil {
		if os.IsNotExist(srcStatErr) {
			return fmt.Errorf(`the src path "%s" does not exist`, src)
		}
		return fmt.Errorf(`call os.Stat on "%s" failed`, src)
	}
	...
	if !srcStat.IsDir() && dstStat != nil && dstStat.IsDir() {
		return fmt.Errorf(`Copy failed: the src path "%s" is file, but the dst path "%s" is folder`, src, dst)
	}
...
func CopyDir(src string, dst string, option ...CopyOption) (err error) {
	...
	if !Exists(dst) {
		if err = os.MkdirAll(dst, usedOption.Mode); err != nil {
			err = fmt.Errorf(`create directory failed for path "%s", perm "%s"`, dst, usedOption.Mode)
			return
		}
	}
	entries, err := os.ReadDir(src)
	if err != nil {
		err = fmt.Errorf(`read directory failed for path "%s"`, src)
		return
	}
